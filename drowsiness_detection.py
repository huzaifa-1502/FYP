                        ...
                        ##########################################################
                        # Driver Drowsiness Detection System - Software Module
                        # Developed for: BS Computer Science Project
                        # Technology Stack: Python, OpenCV, TensorFlow, Keras
                        ##########################################################
...
#helper functions
def send_drowsy():
    global drowsy_sent, awake_sent
    if not drowsy_sent:
        try:
            requests.get(f"http://{ESP32_IP}/drowsy", timeout=0.2)
            drowsy_sent = True
            awake_sent = False
            print("🚨 DROWSY signal sent to ESP32")
        except:
            pass
...
def send_awake():
    ...
    if not awake_sent:
        try:
            requests.get(f"http://{ESP32_IP}/awake", timeout=0.2)
            awake_sent = True
            drowsy_sent = False
            print("✅ AWAKE signal sent to ESP32")
        except:
            pass
...
while(True):
    ...
    for (x,y,w,h) in right_eye:
        ...
        if(np.argmax(right_eye_pred) == 1):
            labels = 'Open' 
        if(np.argmax(right_eye_pred)==0):
            labels = 'Closed'
        break
    ...
    for (x,y,w,h) in left_eye:
        ...
        if(np.argmax(left_eye_pred) == 1):
            labels ='Open'   
        if(np.argmax(left_eye_pred) == 0):
            labels ='Closed'
        break

    if(np.argmax(right_eye_pred) == 0 and np.argmax(left_eye_pred) == 0):
        time += 1
